...
public enum TaskDueDate: String {
  ...
  func dueDateForMoveDate(_ moveDate: Date) -> Date {
    ...
    return Calendar.current.dateByAddingComponents(components, toDate: moveDate, options: .allZeros)!
  }
  
  public func daysFromDueDate(_ moveDate: Date) -> String {
    let taskDueDate = dueDateForMoveDate(moveDate)
    
    let components = Calendar.currentCalendar().components(NSCalendar.Unit.CalendarUnitDay,
